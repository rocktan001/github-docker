...
ROOT = os.getcwd()
ROOT_GROUP = "FVP_RD_N2_CFG1_RD-INFRA-2022_04_04"
MANIFEST_XML_PATH_NAME_RE = re.compile(r"<project\s+path=\"(?P<path>[^\"]+)\"\s+name=\"(?P<name>[^\"]+)\"\s+/>",
                                       re.DOTALL)

gl = gitlab.Gitlab('http://rocktan001/', private_token='sF6uLxucmytTWsM6RJyC')
...
def create_group_and_project():
    ...
    for path in manifest_xml_project_paths:
        ...
        if not is_project_exist:
            ...
            pinfo = gl.projects.create({'name': project, 'path': project, 'namespace_id': group.id})
            print("project create success name=" + project)
            git_path = ["/".join(paths), pinfo.http_url_to_repo]
            git_push_path.append(git_path)
            print(git_path)
            time.sleep(1)

    f = open("git_push_path.txt","w+")
    for p in git_push_path:
        f.write(p[0] + " " + p[1] + "\n")
    f.close()
