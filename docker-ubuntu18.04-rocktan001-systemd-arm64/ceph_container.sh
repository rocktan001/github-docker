...
SSH_PORT=10060
MON_HOST_NAME=admin  # mon/ mgr
OSD_HOST_NAME1=node1 # osd
OSD_HOST_NAME2=node2 # osd
OSD_HOST_NAME3=node3 # osd
OSD_HOST_IP1=10.10.0.102 # 根据子网来设置
OSD_HOST_IP2=10.10.0.103 # 根据子网来设置
OSD_HOST_IP3=10.10.0.104 # 根据子网来设置
...
function create_ceph_osd()
{
	...
	pushd ${OSD_WORKSPACE}
	rm -rf *
	dd if=/dev/zero of=ceph1.xfs bs=1G count=10
	dd if=/dev/zero of=ceph2.xfs bs=1G count=10
	dd if=/dev/zero of=ceph3.xfs bs=1G count=10
	mkfs.xfs ceph1.xfs
	mkfs.xfs ceph2.xfs
	mkfs.xfs ceph3.xfs
...
{
	docker rm -f ${MON_HOST_NAME} ${OSD_HOST_NAME1} ${OSD_HOST_NAME2} ${OSD_HOST_NAME3}

	docker run -tid --name ${MON_HOST_NAME}  \
		--hostname=${MON_HOST_NAME}  --net host \
		-e SSH_PORT=${SSH_PORT} -e ROOT_PWD=root -e YSEMI_USER=ceph \
		--add-host=${MON_HOST_NAME}:${MON_HOST_IP} --add-host=${OSD_HOST_NAME1}:${OSD_HOST_IP1} --add-host=${OSD_HOST_NAME2}:${OSD_HOST_IP2} --add-host=${OSD_HOST_NAME3}:${OSD_HOST_IP3} \
	...
	docker run -tid --name ${OSD_HOST_NAME3}   \
		--hostname=${OSD_HOST_NAME3}  --network=cluster-net --ip=${OSD_HOST_IP3} \
		-e SSH_PORT=22 -e ROOT_PWD=root -e YSEMI_USER=ceph \
		--add-host=${MON_HOST_NAME}:${MON_HOST_IP} --add-host=${OSD_HOST_NAME1}:${OSD_HOST_IP1} --add-host=${OSD_HOST_NAME2}:${OSD_HOST_IP2} --add-host=${OSD_HOST_NAME3}:${OSD_HOST_IP3} \
...
function ceph_ntp()
{
	docker exec -it ${MON_HOST_NAME} /usr/sbin/ntpdate ntp.ubuntu.com
	docker exec -it ${OSD_HOST_NAME1} /usr/sbin/ntpdate ntp.ubuntu.com
	docker exec -it ${OSD_HOST_NAME2} /usr/sbin/ntpdate ntp.ubuntu.com
	docker exec -it ${OSD_HOST_NAME3} /usr/sbin/ntpdate ntp.ubuntu.com
